    ...
    <div className="flex-1 p-8 overflow-y-auto bg-slate-50/30">
      <div className="mb-10">
        ...
        <p className="text-lg text-slate-500 mt-2">
          Quản lý các lựa chọn quản trị và mặc định của ứng dụng.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="flex items-center gap-6">
                <div className="w-24 h-24 rounded-3xl bg-slate-100 border-4 border-white shadow-md relative group">
                  ...
                  <div className="absolute inset-0 bg-black/40 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="text-white text-[10px] font-black uppercase tracking-widest cursor-pointer">
                      Sửa
                    </span>
                  </div>
                </div>
                <div className="flex-1 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        defaultValue="Admin Tổng"
                        className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none ring-2 ring-transparent focus:ring-primary/20 transition-all"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              ...
              {[
                {
                  ...
                  desc: "Bảo vệ tài khoản với xác thực 2 lớp.",
                  active: true,
                },
                {
                  label: "Thông báo Đăng nhập",
                  desc: "Nhận cảnh báo khi có đăng nhập từ vị trí mới.",
                  active: false,
                },
                {
                  label: "Quản lý API Key",
                  desc: "Kiểm soát quyền truy cập của bên thứ ba.",
                  active: true,
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group cursor-pointer hover:bg-slate-100 transition-all"
                >
                  ...
                  <div
                    className={`w-12 h-6 rounded-full relative transition-colors ${item.active ? "bg-primary" : "bg-slate-300"}`}
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${item.active ? "right-1" : "left-1"}`}
                    ></div>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 p-8 rounded-3xl text-white shadow-xl shadow-slate-900/20">
            <h4 className="text-sm font-black uppercase tracking-[0.2em] mb-6 text-slate-400">
              Trạng thái Hệ thống
            </h4>
