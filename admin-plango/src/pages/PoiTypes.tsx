...
import { Search, Plus, Trash2, Edit2, ListTree, Utensils, Coffee, Bed, ShoppingBag, Eye, HelpCircle, RefreshCw, X, Check, Activity } from 'lucide-react';
...
const getVisuals = (name: string) => {
  ...
  if (n.includes('restaurant') || n.includes('nhà hàng') || n.includes('food')) return { icon: 'Utensils', color: '#EF4444' };
  if (n.includes('cafe') || n.includes('cà phê') || n.includes('coffee')) return { icon: 'Coffee', color: '#8B4513' };
  if (n.includes('hotel') || n.includes('khách sạn') || n.includes('stay')) return { icon: 'Bed', color: '#3B82F6' };
  if (n.includes('shopping') || n.includes('mua sắm') || n.includes('market')) return { icon: 'ShoppingBag', color: '#10B981' };
  if (n.includes('attraction') || n.includes('tham quan') || n.includes('sight')) return { icon: 'Eye', color: '#A855F7' };
...
const PoiTypes: React.FC = () => {
  ...
  const [currentType, setCurrentType] = useState<{ poitypeId?: number; typeName: string; description: string }>({ typeName: "", description: "" });
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Bạn có chắc chắn muốn xoá loại địa điểm này? Thao tác này sẽ xóa vĩnh viễn và không thể khôi phục.')) {
      try {
        await apiRequest('DELETE', `/api/poi-types/${id}`);
        setTypes(prev => prev.filter(t => t.poitypeId !== id));
      } catch (error) {
        alert("Xoá thất bại! Có thể loại địa điểm này đang được sử dụng bởi các dữ liệu khác trong hệ thống.");
      }
    }
  };
      ...
      <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
             ...
             <div>
               <h2 className="text-3xl font-black text-slate-800 tracking-tight">Loại địa điểm (POI)</h2>
               <div className="flex items-center gap-2 mt-0.5">
                 <Activity size={12} className="text-primary animate-pulse" />
                 <p className="text-slate-400 font-bold uppercase text-[9px] tracking-widest">POI Categories</p>
               </div>
             </div>
          </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
           {[1,2,3,4,5,6,7,8].map(i => (
             <div key={i} className="h-[180px] bg-white rounded-2xl animate-pulse border border-slate-100 shadow-sm"></div>
           ))}
        </div>
      ) : (
                  ...
                  <div className="h-full bg-white rounded-2xl p-0.5 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                    <div className="p-5 relative z-10 flex flex-col h-full">
                       ...
                       <div className="mt-4 pt-3 border-t border-slate-50 flex items-center justify-between">
                          <span className="text-[8px] font-black text-slate-200 uppercase tracking-widest">POI Registry</span>
                          <span className="text-[10px] font-black text-slate-300 italic group-hover:text-primary/50 transition-colors">#{type.poitypeId}</span>
                       </div>
                    </div>
                  </div>
               </div>
           ...
           <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in slide-in-from-bottom-4 duration-400">
              ...
              <div className="p-6 bg-slate-900 text-white relative">
                  ...
                  <div className="relative z-10 flex justify-between items-center">
                     <div>
                       <h3 className="text-xl font-black mb-0.5">{currentType.poitypeId ? "Chỉnh sửa" : "Tạo mới"}</h3>
                       <p className="text-white/40 font-bold uppercase text-[8px] tracking-widest">POI Type Master Config</p>
                     </div>
                     <button 
              ...
              <div className="p-6 space-y-6">
                  <div className="space-y-2">
                     <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Định danh loại địa điểm</label>
                     <div className="relative">
                        ...
                        <input 
                          ...
                          onChange={(e) => setCurrentType(prev => ({ ...prev, typeName: e.target.value }))}
                  ...
                  <div className="space-y-2">
                     <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Mô tả nội bộ</label>
                     <textarea 
