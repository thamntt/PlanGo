...
import { Search, Plus, Trash2, Edit2, Wallet, Car, Utensils, ShoppingBag, Map, MoreHorizontal, RefreshCw, X, Check, PieChart } from 'lucide-react';
...
const getVisuals = (name: string) => {
  ...
  if (n.includes('di chuyển') || n.includes('xe') || n.includes('transport')) return { icon: 'Car', color: '#3B82F6' };
  if (n.includes('ăn uống') || n.includes('food') || n.includes('restaurant')) return { icon: 'Utensils', color: '#EF4444' };
  if (n.includes('mua sắm') || n.includes('shopping')) return { icon: 'ShoppingBag', color: '#10B981' };
  if (n.includes('tham quan') || n.includes('sightseeing') || n.includes('tour')) return { icon: 'Map', color: '#F59E0B' };
...
const ExpenseCategories: React.FC = () => {
  ...
  const [currentCategory, setCurrentCategory] = useState<{ expenseTypeId?: number; name: string; description: string }>({ name: "", description: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentCategory.expenseTypeId) {
        const updated = await apiRequest('PUT', `/api/expense-types/${currentCategory.expenseTypeId}`, currentCategory);
        setCategories(prev => prev.map(c => c.expenseTypeId === updated.expenseTypeId ? updated : c));
      } else {
  ...
  return (
    <div className="flex-1 p-8 overflow-y-auto bg-[#F8FAFC] min-h-screen">
      ...
      <div className="mb-8 flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-slate-800 tracking-tight">Quản lý Loại chi phí</h2>
          <p className="text-lg text-slate-500 mt-2">Phân loại các khoản chi tiêu trong chuyến đi (từ bảng expensetype).</p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
           {[1,2,3,4,5,6].map(i => (
             <div key={i} className="h-[250px] bg-white rounded-[3rem] animate-pulse border border-slate-100"></div>
           ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredCategories.map((category) => {
            ...
            return (
              <div key={category.expenseTypeId} className="group bg-white rounded-2xl p-0.5 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 relative overflow-hidden">
                <div className="p-5 relative z-10">
                  <div className="flex justify-between items-start mb-4">
                    <div 
                      ...
                      style={{ backgroundColor: visuals.color, boxShadow: `0 10px 20px ${visuals.color}33` }}
                    >
                  ...
                  <div className="mb-4">
                    <h3 className="text-xl font-black text-slate-800 mb-1 group-hover:text-primary transition-colors">{category.name}</h3>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                     <div className="flex items-center gap-2">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">ID: #{category.expenseTypeId}</span>
                     </div>
                  </div>
                </div>
                
                <div className="absolute top-0 right-0 w-20 h-20 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity" style={{ color: visuals.color }}>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-10 bg-slate-900 text-white flex justify-between items-center">
               <h3 className="text-3xl font-black">{currentCategory.expenseTypeId ? "Chỉnh sửa loại chi phí" : "Thêm loại chi phí mới"}</h3>
               <button onClick={() => setIsModalOpen(false)} className="p-4 bg-white/10 hover:bg-white/20 rounded-2xl transition-all">
                  <X size={24} />
               </button>
            </div>
            <div className="p-10 space-y-8">
                <div>
                   <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Tên loại chi phí</label>
                   <input 
                    type="text"
                ...
                <div>
                   <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Mô tả chi tiết</label>
                   <textarea 
            ...
            <div className="p-10 pt-0 flex gap-4">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 py-5 rounded-[2rem] text-lg font-black text-slate-400 hover:bg-slate-50 transition-all">HỦY BỎ</button>
                <button 
                  onClick={handleSave} 
