...
const RoleBadge: React.FC<{ role: UserData["role"] }> = ({ role }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-lg text-xs font-bold ${styles[role] || styles.user} uppercase`}
    >
      {labelMap[role] || role}
    </span>
  );
    ...
    <div className="flex-1 p-8 overflow-y-auto relative">
      <div className="mb-8">
        ...
        <p className="text-lg text-slate-500 mt-2">
          Quản lý quyền truy cập hệ thống và phân quyền người điều hành du lịch.
        </p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="px-8 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest w-1/3">
                Hồ sơ người dùng
              </th>
                ...
                <td className="px-8 py-6 text-right">
                  <div className="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      title={user.isLocked ? "Mở khoá người dùng" : "Khoá người dùng"}
                    >
