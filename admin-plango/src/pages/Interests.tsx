...
import { Search, Plus, Trash2, Edit2, Heart, RefreshCw, X, Check, Filter, Sparkles, Hash } from 'lucide-react';
...
const Interests: React.FC = () => {
  ...
  const [currentInterest, setCurrentInterest] = useState<{ preferenceId?: number; preferenceName: string }>({ preferenceName: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentInterest.preferenceId) {
        const updated = await apiRequest('PUT', `/api/preferences/${currentInterest.preferenceId}`, {
          preferenceName: currentInterest.preferenceName
        });
        setInterests(prev => prev.map(i => i.preferenceId === updated.preferenceId ? updated : i));
      } else {
  ...
  // Generate a random gradient based on ID or Name for visual variety
    ...
    <div className="flex-1 p-8 overflow-y-auto bg-[#F8FAFC] min-h-screen">
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-lg text-slate-500 mt-2">Định nghĩa các loại sở thích du lịch để gợi ý cho người dùng.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        ...
        <div className="flex items-center gap-8 bg-white px-8 py-4 rounded-2xl shadow-sm border border-slate-100">
           <div className="flex flex-col">
             ...
             <span className="text-2xl font-black text-slate-800">{interests.length} <span className="text-sm font-medium text-slate-400">thẻ</span></span>
           </div>
           <div className="w-[1px] h-10 bg-slate-100"></div>
           <div className="flex flex-col">
             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Đang hiển thị</span>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1,2,3,4,5,6,7,8].map(i => (
            <div key={i} className="h-[200px] bg-white rounded-[2rem] animate-pulse border border-slate-100"></div>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filteredInterests.map((interest) => (
            ...
            >
              ...
              <div className={`absolute top-0 right-0 w-16 h-16 bg-gradient-to-br ${getGradient(interest.preferenceId)} opacity-10 rounded-bl-[2rem] group-hover:scale-150 transition-transform duration-500`}></div>
              
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-3">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getGradient(interest.preferenceId)} flex items-center justify-center text-white shadow-md group-hover:rotate-12 transition-transform`}>
                  ...
                  <span className="text-[9px] font-black text-slate-300 tracking-tighter flex items-center gap-0.5">
                    <Hash size={8} />{interest.preferenceId}
                  </span>
                </div>
                
                <h3 className="text-base font-black text-slate-800 mb-4 group-hover:text-primary transition-colors line-clamp-1">{interest.preferenceName}</h3>
                
                <div className="flex items-center justify-between border-t border-slate-50 pt-3">
                   <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Preference</span>
          ...
          {filteredInterests.length === 0 && (
            <div className="col-span-full py-32 flex flex-col items-center justify-center bg-white rounded-[3rem] border-4 border-dashed border-slate-100">
               ...
               <h3 className="text-2xl font-black text-slate-800 mb-2">Không tìm thấy sở thích nào</h3>
               <p className="text-slate-400 font-medium">Hãy thử tìm kiếm với từ khóa khác hoặc thêm mới.</p>
               <button 
                  onClick={openAddModal}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-300">
            {/* Modal Header */}
            <div className={`p-10 bg-gradient-to-br ${currentInterest.preferenceId ? getGradient(currentInterest.preferenceId) : 'from-slate-900 to-slate-800'} text-white relative`}>
               ...
               <div className="relative z-10 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-white/60 font-medium italic">Thông tin thẻ sở thích du lịch</p>
                  </div>
                  <button 
                ...
                <div className="relative group">
                  ...
                  <div className="relative">
                    <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-primary transition-colors">
                       <Heart size={24} fill={currentInterest.preferenceName ? 'currentColor' : 'none'} />
                    </div>
                    <input 
                      type="text"
                      value={currentInterest.preferenceName}
                      onChange={(e) => setCurrentInterest(prev => ({ ...prev, preferenceName: e.target.value }))}
