...
import { Search, Plus, Trash2, Edit2, Tags, Compass, Palmtree, Mountain, Hotel, Landmark, RefreshCw, X, Check, MapPin } from 'lucide-react';
...
const getVisuals = (name: string) => {
  ...
  if (n.includes('biển') || n.includes('island') || n.includes('beach')) return { icon: 'Palmtree', color: '#06B6D4' };
  if (n.includes('núi') || n.includes('mountain') || n.includes('highland')) return { icon: 'Mountain', color: '#10B981' };
  if (n.includes('nghỉ dưỡng') || n.includes('resort') || n.includes('spa')) return { icon: 'Hotel', color: '#F43F5E' };
  if (n.includes('di tích') || n.includes('historical') || n.includes('culture')) return { icon: 'Landmark', color: '#F59E0B' };
...
const DestinationTypes: React.FC = () => {
  ...
  const [currentType, setCurrentType] = useState<{ destinationtypeId?: number; typeName: string; description: string }>({ typeName: "", description: "" });
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Bạn có chắc chắn muốn xoá danh mục điểm đến này? Thao tác này sẽ xóa vĩnh viễn.')) {
      try {
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentType.destinationtypeId) {
        const updated = await apiRequest('PUT', `/api/destination-types/${currentType.destinationtypeId}`, {
          typeName: currentType.typeName,
          description: currentType.description
        });
        setTypes(prev => prev.map(t => t.destinationtypeId === updated.destinationtypeId ? updated : t));
      } else {
    ...
    <div className="flex-1 p-8 overflow-y-auto bg-[#F8FAFC] min-h-screen">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-base text-slate-500 font-medium italic">Phân loại các khu vực du lịch (Thành phố, Biển đảo...)</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
          ...
          {filteredTypes.length === 0 && (
            <div className="col-span-full py-32 flex flex-col items-center justify-center bg-white rounded-[4rem] border-4 border-dashed border-slate-100">
               ...
               <h3 className="text-3xl font-black text-slate-800 mb-3">Không tìm thấy danh mục nào</h3>
