...
const ActivityItem: React.FC<ActivityItemProps> = ({ type, title, description, timestamp, rating, avatar }) => {
  const IconMap = {
  ...
  return (
    <div className="flex items-center gap-4 py-6 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors px-4 group cursor-pointer">
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 overflow-hidden ${avatar ? '' : BgMap[type]}`}>
        {avatar ? (
      ...
      <div className="flex items-center gap-4">
        {rating && (
          <div className="flex gap-0.5">
            {[...Array(5)].map((_, i) => (
              <Star key={i} size={14} className={i < rating ? "fill-[#B45309] text-[#B45309]" : "text-slate-200"} />
            ))}
          </div>
        )}
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex justify-between items-center">
        ...
        <button className="text-sm font-semibold text-[#0891B2] hover:text-[#0E7490] transition-colors">Xem tất cả</button>
      </div>
      <div>
