    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
        ...
        <div className="relative group min-w-[140px]">
          <Calendar
            className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-primary transition-colors pointer-events-none"
            size={14}
          />
          <select
            value={timeFilter}
      ...
      <div>
        {popularDestinations.length > 0 && popularDestinations.some((d) => d.selectionCount > 0) ? (
          popularDestinations.map((dest) => (
            <div
              key={dest.id}
              className="flex items-center gap-4 py-6 border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors px-6 group cursor-pointer"
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-bold text-slate-800 tracking-tight group-hover:text-primary transition-colors">
                    {dest.name}
                  </p>
                  <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-medium uppercase">
                    {dest.category}
                  </span>
                </div>
                <div className="flex items-center gap-3 mt-1.5">
                  ...
                  <div className="flex items-center gap-1 text-slate-400">
                    ...
                    <span className="text-xs">
                      {dest.address ? dest.address.split(",").pop()?.trim() : "Việt Nam"}
                    </span>
                  </div>
                </div>
              </div>
              <div className="text-right flex flex-col items-end gap-1">
                ...
                <p className="text-[10px] text-slate-400 font-medium">
                  {dest.reviewCount} đánh giá
                </p>
              </div>
              <ChevronRight
                size={18}
                className="text-slate-200 group-hover:text-slate-400 transition-colors ml-2"
              />
            </div>
          ))
        ) : (
          <div className="p-16 text-center flex flex-col items-center gap-4">
            ...
            <p className="text-slate-400 italic">
              ...
              {timeFilter === "week" ? "tuần" : timeFilter === "month" ? "tháng" : "năm"} này
            </p>
          </div>
        )}
      </div>
